...
# DEFAULT_GENERATE_KWARGS = {"max_new_tokens": 512, "do_sample": False} # default
# NOTE: whatever value `temperature` or `top_p` is set, it won't affect the final generation
# as long as `do_sample` is False. we set them as None to avoid warnings.
...
class ContextCiter:
    def __init__(
        ...
        if self.tokenizer.pad_token is None:
            print(f"self.tokenizer.pad_token is None; set `pad_token` the same as `eos_token`")
            self.tokenizer.pad_token = self.tokenizer.eos_token
        
        print(f"[INFO] Initializing `ContextCiter` from local customized context_cite")
    ...
    def _output(self):
        if self._cache.get("output") is None:
            ...
            output_ids = self.model.generate(input_ids, pad_token_id=self.tokenizer.pad_token_id, **self.generate_kwargs)[0]    # manually set pad_token_id to cancel warning
    ...
    @property
    def response_with_indices(self, split_by="word", color=True) -> List[str | pd.DataFrame]:
        """
