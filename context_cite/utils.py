...
def split_text(text: str, split_by: str) -> Tuple[List[str], List[str], List[str]]:
    ...
    # TODO: for contexts with knowledge points
    kp_tag = "### Knowledge Point "
    if split_by == "sentence":
        if kp_tag not in text:
            # normal case: same as default
            for line in text.splitlines():
                parts.extend(nltk.sent_tokenize(line))
        else:
            # w/ knowledge points: coarse-grained partition
            kp_splits = text.split(kp_tag)
            ## 1. question_context: description of patient
            question_context = kp_splits[0]
            for line in question_context.splitlines():
                parts.extend(nltk.sent_tokenize(line))
            ## 2. kp_context: intro to relevant kps
            kp_context = [(kp_tag + kp).strip() for kp in kp_splits[1:]]
            parts.extend(kp_context)
    elif split_by == "word":
        tokenizer = English().tokenizer
        parts = [token.text for token in tokenizer(text)]
    else:
